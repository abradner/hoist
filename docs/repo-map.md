...
|---|---|---|---|
| TUI / CLI | `cmd/hoist` | Local user; whoever runs the binary | The only human input. Keypresses in the plan/confirm, deploy-confirm and flight screens are the authorisation for everything below. Since M10 the matrix also *lists* what is in flight (`internal/app/matrix/inflight.go`, fed by `cmd/hoist/wiring.go`'s `buildInFlightFuncs`): the same `engine.ListStates` + `ObserveSteps` + `Status` path `hoist promotions` walks, re-observed against the forge and cluster at boot and every `poll.approval`, and `r` re-drives one exactly as `hoist resume <id>` does — a read of state files plus remote observation, never a write, until the flight screen it opens takes over. `internal/parity` holds the two faces to the same operation set. |
...
| GitHub API | `pkg/forge/github` | `gh` auth token via go-gh | Opens PRs, reads checks and comments, **merges**, **closes** (`ClosePR`, `hoist abandon` — the only other state transition this adaptor writes, PATCHing `state: closed`; abandon's branch deletion is a `pkg/git` write over the operator's own SSH, the same edge `MergePR`'s own branch delete already uses, never a second forge call), and (M6) lists a repo's git tags with commit dates for the tag picker's ordering (`Tags`) — read-only, bounded to `maxTagPages`. Merge = deploy on an auto-sync repo. It reads no *reviews* — `pkg/forge.Forge` has no review method and no `pulls/*/reviews` path exists — so the approval gate is a comment gate, which is why hoist acting as the operator is workable at all today (#90); and it creates no branches: those are `pkg/git` over the operator's own SSH, not forge calls. Since M10 the same adaptor is also built against each **app** repo named in `repos[].apps` (`cmd/hoist/history.go`), for six read-only history methods — `ResolveRef`, `Compare`, `CommitFiles`, `CommitsTouching`, `BlameLines` (GraphQL), `ReadFile` (the app repo's `.hoist.yaml`) — that `pkg/migrate` turns into the commit and migration delta the confirm screens show (R-005). That is a new trust edge in two directions: the app repo may be private or in another org, so a forge that cannot be built or a `FORBIDDEN`/`NOT_FOUND` answer degrades to a named gap on the screen, never a failed plan; and commit subjects, bodies, authors and file paths are upstream text, passed through `pkg/redact` at decode, and never reach a PR body or commit hoist writes. `BlameLines` on the *gitops* repo dates each manifest line for "live N days"; on a 404 (an unpushed HEAD) it falls back to the base branch and says the age is approximate. Bounded to three pages each of a compare, a commit's file list and the commits-touching list, each reporting when the bound bit so `pkg/migrate` shows the migration count as a floor, and one blame query per manifest occurrence asked about (`BlameLines` takes a list of lines, but today's caller asks one line at a time). |
...
- **What is in flight (M10).** The matrix's in-flight pane and `hoist promotions` share one
  path: read every state file under `$XDG_STATE_HOME/hoist/promotions/` as an *index* of where
  to look, then re-observe each one's git/forge steps (`engine.ObserveSteps`) and summarise
  (`engine.Status` → `flight.Summarize`) — the phase a state file recorded is never displayed
...
| R-005 | An image promotion is also a schema migration (the app's entrypoint runs `db:prepare`), so a merge can migrate production as a side effect | `pkg/migrate` (`Resolver`, `Comparer`, `Blamer`, `MigrationsPath`), `pkg/forge` (`ResolveRef`, `Compare`, `CommitFiles`, `CommitsTouching`, `BlameLines`, `ReadFile`), `internal/app/history`, `cmd/hoist/history.go` | **Built (M10, data path); surfaced by the M10 screens that follow.** The delta between the build an env runs and the build about to be written is computed against the *app* repo's forge: image tag → revision (the `org.opencontainers.image.revision` label, else a git tag of the same name, else a `sha-<prefix>` tag, else unknown — the source is reported, since a tag is mutable and a label is not), then a three-dot compare with migration files attributed to the commits that added them under a per-app path prefix (the app repo's own `.hoist.yaml`, else `repos[].migrations`, else `db/migrate/`). A rollback (target behind source) is compared the other way and lists the migrations being *un-applied* — a forward compare would report zero commits for the most dangerous case. Never blocks: the gitops repo's runbook owns the "never bundle a promotion with a manual migration" rule; hoist makes the delta visible. A delta that cannot be computed is a typed `ErrUnresolved` naming the missing end and why (no `apps` mapping, no revision, revision not in the app repo), never an empty list that reads as "no commits". A forge scope failure is an error, never folded into "unknown". Trust boundary: commit subjects, bodies, authors and paths are upstream text and pass `pkg/redact` at the adaptor; the app repo may be private in another org, so a forge that cannot be built degrades to "unmapped" rather than failing the screen. |
...
| R-007 | Direct mode (M6) is a new kind of write surface: a commit straight to a target env's base branch, with no PR, no CI gate, and no approval comment — every review step the normal promotion flow drives never runs at all | `internal/engine/direct.go` (`DirectCommitGateStep`, `DirectSteps`, `AllDirectSteps`), `cmd/hoist` (`--direct`/`--confirm-direct` on both `promote` and `deploy`), `internal/app/tags` (the `D` keypress + `huh.Confirm` gesture) | Gated solely on "target env not listed in `envs.production`" — the same list that already governs PR-required and approval-required elsewhere (§4.5), never a second, independent toggle. `DirectCommitGateStep` runs first in `DirectSteps`, so a block there leaves every later write step unreached through `Drive` — and it stays first in `AllDirectSteps`, the M8 list that appends the Argo/rollout tail, so extending direct mode past the push did not move the gate. It is deliberately absent from `engine.ObserveSteps`, the list a PRIOR direct state is *observed* by (`findInFlight`, `hoist promotions`, `resume --env`): the gate decides whether a direct promotion may start, and re-running it against one that already landed would let a later config edit re-interpret a finished run as blocked, which is a state file read through today's config rather than observed. Both `Observe` and `Act` independently re-derive the refusal rather than trusting the caller's belief (`TestDirectModeRefusesProductionEnvByConstruction`, `internal/engine/direct_test.go`). The residual risk is entirely in this invariant staying single-sourced: `DirectSteps`' own doc comment states `ProductionEnvs` must always be the repo's full, unfiltered `envs.production` list, never pre-filtered by a caller, and that no second "allow direct anyway" config field exists or should be added. A future change that weakens either of those — a per-env override that bypasses the check, or a caller passing a narrowed env list — would reopen exactly the config-bug risk this step exists to make structurally impossible, and would do so silently unless it is read against the doc comment's stated invariant rather than treated as a plausible enhancement. |
| R-008 | `hoist deploy` (M8) writes an operator-named image ref into an env with no source env to have vouched for it — the staging→production pairing that makes a promotion inherently second-hand is absent by construction, so a never-deployed tag can be written straight into production | `pkg/gitops` (`BuildDeployPlan`, `WarnProductionTarget`), `cmd/hoist/deploy.go`, `internal/app/deploy`, `internal/app/tags` (`stagingNote`) | **Warned, not blocked (principle 5), and gated the same way §4.5 already requires.** A production target still means a PR — unconditionally — and an outright refusal of direct mode. The human approval comment on top of that is the *default*, not a guarantee: an explicit per-env `approval: auto` is permitted (§4.5, `RepoConfig.Approval`), so the mandatory half of this gate is the PR and the direct refusal, and the comment is whatever the repo configured — `deploy` shares `promote`'s `checkDirectPreflight` and the same `DirectCommitGateStep`, so this path adds no second decision about what production means. On top of that: `WarnProductionTarget` is a real plan warning, so the dry run, the confirm screen and the PR body all name it (an earlier UI-only string reached none of the three, including the PR the human is meant to review); every deploy shows a diff and takes an explicit Enter before anything is claimed or written; and the tag picker compares the tag under the cursor against the paired staging env's *committed manifest* tags, warning when that tag is not among them. That comparison is deliberately not a live-cluster read — `internal/app/tags` has no cluster connection wired in at all — and its wording says "committed", so it never claims a rollout finished that Argo may not have started. Two limits are worth stating exactly, because a risk map that overstates a safeguard is worse than one that omits it. It compares tag *strings*, not digests: a tag is mutable, so a match means staging committed that tag, not that staging ran this build — and the note says so rather than letting the operator read more into it. And it renders only when the paired staging env has some occurrence of this image repo; when staging has never carried the repo at all — the strongest case of all — there is no note, so that case is currently uncovered rather than warned. Both are issue #74. The residual risk is that all four of these inform rather than block: an operator who reads none of them can still deploy an unproven tag to production through a PR they approve themselves — or, on an env configured `approval: auto`, through one nobody comments on at all. |
| R-009 | `hoist restart` writes to the live cluster with nothing committed to git: no PR, no CI, no approval comment, no reviewable record, and no state file — every review step the promotion flow drives is absent by construction, because there is no change to review | `pkg/rollout` (`Restart`, the package's only write), `internal/restart` (the shared core), `cmd/hoist/restart.go`, `internal/app/restart` (the matrix's `R`) | **Bounded by what the operation can do rather than by review.** A restart changes *nothing about what is declared*: it stamps one pod-template annotation, which rolls the existing pods onto the same image references. Onto the same image *builds* only where those references are pinned — a mutable tag can resolve to a different digest when the replacement pod lands, which is why every unpinned reference is named among the warnings rather than assumed away. There is no content to review, which is why the §4.5 PR-and-approval gate does not apply and is not simulated. What stands in its place: the target set is named before anything is written (with each Deployment's replica count, strategy and last restart), a production env additionally requires `--confirm-production=<env>` repeating the env exactly — the `--confirm-direct` shape, so a production restart is never one keystroke — and every reason the restart will not be graceful (`replicas: 1`, no readiness probe, `strategy: Recreate`) is surfaced per Deployment first, warning and never blocking (principle 5). Targets come from the GitOps repo's family declarations, never from listing the namespace: a namespace holds several families' workloads, so listing would restart far more than asked. **Deliberately NOT through git.** An earlier design wrote the annotation into the manifest and drove the full promotion pipeline, on the belief that Argo's `selfHeal: true` would revert a live patch. That belief was false — Argo's diff is a three-way merge, so a field it never set and that is absent from the manifest is left alone, the same way `kubectl apply` does not delete fields it never wrote — and it was disproved by the target cluster itself, where annotations added this way have survived weeks on self-healing Applications reporting Synced. The residual risk is the honest one: a production restart is a real, unreviewed action on running pods, gated only by the operator's own second acknowledgement. Both surfaces go through `internal/restart`, so the CLI and the `R` key cannot drift on what a restart targets, what it warns about, or how the write is confirmed. Production's second gesture is `--confirm-production=<env>` at the CLI and a `huh.Confirm` on the screen. |
| R-010 | `hoist abandon` (M11) retires a promotion that never landed: it can close the PR, delete the remote branch, and delete the state file — a new destructive surface with no rollback of its own | `pkg/forge/github` (`ClosePR`), `pkg/git` (`DeleteRemoteBranch`), `internal/engine` (`DeleteState`, `ArchiveState`), `cmd/hoist/abandon.go`, `internal/app/flight` (the `X` gesture) | **Gated on re-observation, not the caller's belief (§4.1).** `abandonPromotion` re-runs `engine.ObserveAll` (plus a direct `LandedSHA()` check, since a merged-but-not-yet-Argo-converged promotion can already carry a landed SHA `ObserveAll`'s own coarser `done` misses) before touching anything, and refuses outright if the promotion has landed — abandoning is explicitly not a rollback; what merged stays merged. The write itself needs a second, distinct confirming argument repeating the id (`--confirm-abandon=<id>` at the CLI, the same `huh.Confirm` shape as `--confirm-direct`/`--confirm-production` at the flight screen's `X`). State retention (`hoist promotions` auto-archiving terminal states past `state.retain`) is the adjacent, non-destructive half of the same lifecycle: archiving only runs once re-observation says terminal, never on age alone, so it can never be the thing that drops the one-in-flight-per-env lock (the failure mode #166 was about). The TUI's `X` gesture cancels the flight screen's shared drive context, then waits for `Busy()` to actually clear (`internal/app`'s `abandonWaitMsg`/`doAbandon`) before firing the abandon write at all — a plain context cancel is not a join, and `engine.Drive` saves state (and can still complete a push/merge) after a step's `Act` returns even on a canceled context, so firing the write the instant `Cancel` is called could race a drive that is mid-`Act`, leaving the state either recreated or the remote mutated after abandon believed it was done (flagged by an independent review of PR #182). The wait is bounded (`abandonWaitMaxAttempts`, ~1s at 100ms polls) rather than unconditional, precisely because `abandonPromotion`'s own re-observation is the real backstop: a `DriveFunc` that somehow never notices cancellation within that window still gets caught by the `ObserveAll`/`LandedSHA()` check above rather than wedging the operator's keypress forever, so the residual risk is narrower than "not yet closed" — a genuinely-landed race is still refused, only a still-in-flight one that outlasts the wait proceeds on the wait's own honest warning notice rather than a confirmed-safe silence. |
