...
func TestBuildHistoryFuncsLabelWinsAndPolicyIsRead(t *testing.T) {
	...
	h := buildHistoryFuncs(cfg, rc, &gitops.Repo{Root: "/x"}, &forge.Fake{}, nil, "head")
...
func TestBuildHistoryFuncsUnmappedRepoIsUnresolved(t *testing.T) {
	...
	h := buildHistoryFuncs(nil, rc, nil, nil, nil, "")
...
func TestBuildHistoryFuncsLiveAgeWithoutForge(t *testing.T) {
	...
	h := buildHistoryFuncs(nil, rc, nil, nil, errors.New("gh not logged in"), "head")
	...
	h = buildHistoryFuncs(nil, rc, nil, gitopsForge, nil, "head")
