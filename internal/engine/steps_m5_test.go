...
// TestRolledOutDoesNotGateOnASupersededImage is Codex's round-2 P1 on PR #167: the supersede
// verdict removed the wedge from DirectPushedStep and ArgoSyncedStep, and left it standing one
// step later.
//
// RolledOutStep compares every live container against this promotion's own Edit.New. Once a
// later change has superseded the promotion at the base, the cluster is converging on that newer
// reference and Edit.New is a value no container will ever carry again — so the step waited
// forever, the promotion never reached a terminal phase, and `hoist promotions` and the in-flight
// pane showed it as live work indefinitely. Which is bug #165's symptom exactly, relocated rather
// than closed.
func TestRolledOutDoesNotGateOnASupersededImage(t *testing.T) {
	s, fx := gitBackedArgoState(t)
	g := git.Exec{}
	newer := "ghcr.io/example/app:v3@sha256:" + strings.Repeat("2", 64)
	tip := supersedeBase(t, fx, newer)
	// LandedSHA is the base revision that actually carries what the env declares now, which is
	// what every step past the push converges against (DirectPushedStep's own doc comment).
	s.MergeSHA = tip

	// The cluster is running the SUPERSEDING image, not this promotion's.
	ro := &rollout.Fake{}
	for _, e := range s.Edits {
		if e.Kind != "Deployment" {
			continue
		}
		ro.SetDeployment(s.TargetEnv, e.Name, rollout.DeploymentStatus{
			Namespace: s.TargetEnv,
			Name:      e.Name,
			Images:    []rollout.ContainerImage{{Name: e.Container, Image: newer}},
			Complete:  true,
			Detail:    "deployment successfully rolled out",
		})
	}

	obs, err := (RolledOutStep{Rollout: ro, Git: g}).Observe(ctx(), s)
	if err != nil {
		t.Fatalf("Observe: %v", err)
	}
	if !obs.Satisfied {
		t.Fatalf("a superseded promotion cannot ever see its own image live again; gating on it waits forever: %+v", obs)
	}
	if !strings.Contains(obs.Detail, "superseded") {
		t.Errorf("Detail should say why this promotion stopped gating on its own images, got %q", obs.Detail)
	}
}

// TestRolledOutStillGatesOnAnImageThatSimplyHasNotLandedYet is the control: the supersede escape
// hatch must not turn every mismatch into success. An ordinary not-yet-rolled-out Deployment —
// the base still declares exactly what this promotion wrote — keeps waiting.
func TestRolledOutStillGatesOnAnImageThatSimplyHasNotLandedYet(t *testing.T) {
	s, _ := gitBackedArgoState(t)
	ro := &rollout.Fake{}
	for _, e := range s.Edits {
		if e.Kind != "Deployment" {
		...
		ro.SetDeployment(s.TargetEnv, e.Name, rollout.DeploymentStatus{
			Namespace: s.TargetEnv,
			Name:      e.Name,
			Images:    []rollout.ContainerImage{{Name: e.Container, Image: e.Ref.String()}},
			Complete:  true,
			Detail:    "deployment successfully rolled out",
		})
	}
	obs, err := (RolledOutStep{Rollout: ro, Git: git.Exec{}}).Observe(ctx(), s)
	if err != nil {
		t.Fatalf("Observe: %v", err)
	}
	if obs.Satisfied {
		t.Fatalf("the base still declares this promotion's own image; a container still on the old one is not rolled out: %+v", obs)
	}
	if !obs.Waiting {
		t.Errorf("Observe = %+v, want Waiting (retryable)", obs)
	}
}
