...
func defaultKeyMap() keyMap {
	return keyMap{
		...
		// Capital R, deliberately: a restart rolls every pod of a family with no image change,
		// which is a write, and the lower-case neighbours here are all "open a screen". The
		// screen it opens is still a confirm, so this is not the gesture that does it — it is
		// the gesture that asks.
...
// what it rolls is every Deployment that family declares, which is the unit an Argo
// Application already covers and therefore the unit the rollout is watched at.
