...
// than an image because a restart changes no image: what it rolls is every Deployment the
// family declares, which is the unit an Argo Application already covers.
func TestRestartKeyEmitsOpenRestartMsg(t *testing.T) {
	...
	if msg.Family != m.CurrentFamily() {
		t.Errorf("Family = %q, want the family under the cursor %q", msg.Family, m.CurrentFamily())
	}
	if msg.Family == "" {
		t.Error("fixture precondition: the cursor should be on a family")
	}

	// Lower-case r is not it: the restart key is deliberately capital, since it writes.
